...
case class DataNode(host: String, port: Int, props:  Config = null  )
case class NameNode(host: String, port: Int, dataNodes: List[DataNode], props: Config = null)
case class JobTracker(host: String, port: Int, dataNodes: List[DataNode], props: Config = null)
case class ClusterTopology(nameNode: NameNode, jobTracker: JobTracker, props: Config = null)


/***
 * So this version we have 
 * namenode 1
 * jobtracker 1
 * datanode *
each node is an actor system which simulator a real machine, a real node in hadoop cluster
 * 
 */
